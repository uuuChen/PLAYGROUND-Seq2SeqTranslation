...
class Decoder(nn.Module):
    def __init__(self, output_size, embedded_size, hidden_size, sos_idx, eos_idx, teacher_forcing_ratio, device):
    ...
    def forward(self, encode_vec, targets, is_test=False):
        batch_size = encode_vec.shape[1]
        max_target_len = targets.shape[0] if not is_test else 1000  # 1000 is hyper-parameter
        ...
        # only use teacher forcing when training
        use_teacher_forcing = (True if random.random() > self.teacher_forcing_ratio else False) if not is_test else False
